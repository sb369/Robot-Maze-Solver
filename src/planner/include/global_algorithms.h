...
vector<Index> HAstar_park::search_path(const Index& src, const Index& dest){
    ...
    //Node matrix[maplen];
    vector<vector<Node_park>> matrix3d(maplen,vector<Node_park>(total_orientations));
    int i = src.pos;
...
vector<Index> HAstar_park::findNeighbours(const Index& index){
    ...
    for(int counter_i=0; counter_i<(sizeof(dd)/sizeof(dd[0]));counter_i++){
        for(int counter_j=0; counter_j<(sizeof(steer_angles)/sizeof(steer_angles[0])); counter_j++){
            ...
            if(isInside(neighbour_y,neighbour_x) && isNotBlocked(neighbour_y*width + neighbour_x) && (!(neighbour_x==current_x && neighbour_y==current_y && new_theta==index.orientation))){
                neighbours.push_back({neighbour_y*width + neighbour_x,new_theta});
